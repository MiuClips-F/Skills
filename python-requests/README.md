...
本 skill 复用当前已登录页面时，优先通过 chrome-devtools-mcp 附着用户已经打开并登录好的 Chrome 标签页。只有 MCP 路径失败或请求元数据不足时，才下移到直接 CDP，此时才需要 Chrome 开启远程调试端口 `9222`。
...
如果需要走直接 CDP，再在这个 Chrome 里登录目标系统，打开需要抓包的目标页面。

## 标准操作步骤

1. 打开已登录的目标页面。
2. 优先确认 chrome-devtools-mcp 能看到目标标签页；只有 MCP 路径失败或元数据不足时，才确认 Chrome 调试端口可访问：`http://127.0.0.1:9222/json/version`。
...
- 优先用 chrome-devtools-mcp 接管已经打开的 Chrome 和已有标签页。
- 只有 MCP 路径失败或请求元数据不足时，才检查 `127.0.0.1:9222` 是否可用。
- 如果 chrome-devtools-mcp 只看到 `about:blank`、空白页或受管新会话，不能把它当成用户当前页面。
- 遇到受管新会话时，必须停止 MCP 路径并改走直接 CDP 附着。
...
1. chrome-devtools-mcp 附着已有 Chrome 标签页。
2. 直接 CDP 附着已有 Chrome `9222` 标签页。
3. 能暴露同一用户 profile、目标标签页或可验证登录态的 CloakBrowser。
4. 停止并说明无法复用当前会话。
...
- 最终 CSV 按 `数据抓取时间-业务日期-业务导出表名称.csv` 命名，例如 `20260616235435-20260615-已购客核心指标.csv`，并用 UTF-8 保存。
- 店铺 ID、店铺名、商品 ID、SKU、商品名称、直播间名称、直播场次 ID、账户名、账户 ID、文件名等关键字段写入文件内容，不写入文件名。
- 非系统自生成的文本、CSV、JSON、字段映射、任务状态和脚本文件统一使用 UTF-8 编码。

原始下载场景：

- 只保存服务端直接返回的 `csv`、`xlsx`、`zip`、`pdf` 或其他二进制文件。
- 不生成 `field_mapping.json`。
- 文件名前缀同样使用 `数据抓取时间-业务日期-业务导出表名称`，但保留服务端原始后缀。
...
- 多子任务采集时使用 `task_state.json` 记录状态，下次重跑自动跳过已完成子任务。
...
- 强制按 Ponytail 规范生成，优先最少文件、最少函数、最少状态。
- 默认只保留 1 个业务 `class`。
- 遵守高内聚、低耦合原则，一个完整功能优先在一个方法内完成。
- 禁止抽离无关的小方法、通用 `utils`、通用 client、service、factory 或插件式框架。
...
- 一个大任务包含多个报表、店铺、日期分片或子采集脚本时，使用 `task_state.json`。
...
这说明 chrome-devtools-mcp 没有附着到用户已经打开的 Chrome 标签页。不要继续在这个空白页里导航目标站点，应切换到 `127.0.0.1:9222` 的直接 CDP 附着；如果仍失败，需要重新用 9222 启动 Chrome。
...
- 表格导出是否按页面表头落表。
- 原始下载是否没有生成字段映射。
- 输出文件是否按 `数据抓取时间-业务日期-业务导出表名称` 拆分命名，关键业务维度是否写入文件内容，爬取数据是否为 UTF-8 CSV，下载报表是否保留原后缀。
- 最终脚本是否移除了 chrome-devtools-mcp、CDP 和 9222 逻辑。
