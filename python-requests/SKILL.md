...
- 每个采集大任务的业务输出必须按 `output/<shop_id>/<shop_name>_<yyyyMMddHHmmss>_<业务功能名称>.<suffix>` 生成；用户口头写成 `shop_id/shop_name_yyyymmddmiss_业务功能名称.csv` 时，`shop_id/` 按子目录处理，时间戳按 `%Y%m%d%H%M%S` 生成。
- `shop_id` 作为目录段，`shop_name` 和 `业务功能名称` 作为文件名段；只清理文件系统不允许的字符，业务功能名称保留用户能识别的名称。
...
- 同一脚本包含多个采集请求任务、多个报表或多个业务功能时，必须按请求任务或业务功能拆分成多个输出文件；除非用户明确要求汇总，不要把多个业务输出合并成同一个 CSV。
...
2. 关键参数包括但不限于：店铺 ID、店铺名、cookie、token、日期范围、分页大小、业务功能名称、导出路径、输出文件名；只有表格导出场景才包含字段映射路径。
...
- 采集大任务输出按 `output/<shop_id>/<shop_name>_<yyyyMMddHHmmss>_<业务功能名称>.<suffix>` 命名；爬取数据写 UTF-8 CSV，下载报表保留原始后缀
