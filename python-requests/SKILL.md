...
description: 根据已登录浏览器中的真实操作链生成简洁的 Python requests 脚本。优先通过 chrome-devtools-mcp 附着用户已打开的 Chrome 和现有标签页做抓包与请求链分析；如果 MCP 只能提供受管新浏览器、空白页、独立会话或请求元数据不足，才改走直接 CDP 附着，仍无法附着时立即停止，不能把新打开的浏览器当成用户当前页面。适用于需要从现有浏览器会话逆向接口、复刻 Web 操作、分析 headers/cookies/tokens/pagination/downloads、抽取加密参数或生成请求脚本的任务。
---
...
复用当前已登录页面时，先用 chrome-devtools-mcp 查看和选择现有标签页；只有 MCP 只能暴露受管新会话、页面列表里只有新的 `about:blank`、空白标签页、必须调用 `new_page` 才能看到目标站点，或拿不到足够请求元数据时，才下移到直接 `127.0.0.1:9222` 的 CDP 附着。如果直接 CDP 仍无法附着，就停止并告知用户。

CloakBrowser、Scrapling、chrome-devtools-mcp 和 CDP 的优先级必须按任务场景判断，不能写成全局固定顺序。当前任务要求复用“用户已登录的当前页面”时，真实会话优先，先用 chrome-devtools-mcp 附着和验证现有标签页，失败或元数据不足时再下移到直接 CDP；只有 CloakBrowser 明确暴露同一个用户已登录 profile、目标标签页或等价可验证会话时，才可作为该场景的接入口。当前任务要求新建受控浏览器、处理指纹敏感页面或先完成可授权登录时，优先尝试 CloakBrowser，其次尝试 Scrapling，最后再退回普通 CDP/浏览器路径。Scrapling 更适合无须复用当前真实标签页的抓取、动态渲染和用户显式传入认证参数的场景，不应被描述为“已接管当前页面”。
...
1. 先判断任务属于“复用当前已登录页面”还是“新建受控浏览器会话”。复用当前页面时，优先用 chrome-devtools-mcp 选择和验证用户已打开的 Chrome 标签页；MCP 不可用、只看到受管空白页或请求元数据不足时，再下移到 `127.0.0.1:9222` 直接 CDP；新建受控会话时，按 CloakBrowser -> Scrapling -> 普通 CDP/浏览器路径尝试。
2. 复用当前页面时，先用 chrome-devtools-mcp 读取已打开页面列表并匹配目标标签页；只有 MCP 路径失败或不足时，才检查 `http://127.0.0.1:9222/json` 或 `http://127.0.0.1:9222/json/version`，只接管已打开的浏览器和已有标签页，禁止新开浏览器或新起 Chrome 实例。
3. 确认目标标签页、当前 URL、登录状态，以及用户给出的图片或文字操作路径是否足够明确。
4. 先验证 chrome-devtools-mcp 是否真的附着在该已有标签页上；如果只能看到新的 `about:blank`、空白受管页、独立会话，或需要新开页才能继续，立即停止使用该 MCP 会话并改走直接 CDP 附着。
5. 仅在确认已附着到用户现有标签页后，再优先用 chrome-devtools-mcp 执行用户描述的操作；直接 CDP 只作为 MCP 不足时的后备路径。优先用页面快照定位元素，再执行点击、填写、上传、按键和等待。
...
- 优先用 chrome-devtools-mcp 附着已打开的 Chrome 标签页。只有在已经确认 MCP 看到的就是该现有标签页时，才使用页面选择、快照、点击、填写、等待、控制台或网络检查、截图和下载确认。
- 如果 chrome-devtools-mcp 显示的是受管新浏览器、孤立会话，或页面列表里只有新的 `about:blank`，不要把它当成目标页面；改走直接 `http://127.0.0.1:9222/json` 和匹配标签页的 `webSocketDebuggerUrl`。
...
- 如果 chrome-devtools-mcp 暴露不了完整请求头或请求体，改用 Chrome DevTools Protocol：访问 `http://127.0.0.1:9222/json`，附着匹配标签页的 `webSocketDebuggerUrl`。
- 只有 MCP 路径失败或元数据不足时，才确认 9222 端口并走直接 CDP。禁止为了抓包而新开浏览器。
...
- 遇到“未附着”时，不要继续对该受管会话做点击、导航或抓包。MCP 路径未附着时才切换到直接 CDP 附着；仍失败则停止并向用户报告阻塞原因。
...
- 生成表格文件名只使用 `数据抓取时间-业务日期-业务导出表名称.<suffix>`，例如 `20260616235435-20260615-已购客核心指标.csv`；数据抓取时间按 `%Y%m%d%H%M%S` 生成，业务日期优先使用源表或接口中的业务日期并格式化为 `%Y%m%d`，没有源业务日期时再用用户传入的统计日期。
- 文件名不要包含店铺 ID、店铺名、商品 ID、SKU、商品名称、直播间名称、直播场次 ID、账户名、账户 ID 等业务维度；这些关键字段必须写入文件内容。
- 表格内容必须优先补齐统一字段：数据日期、店铺ID、店铺名、商品ID、SKU、商品名称、直播间名称、直播场次ID、账户名、账户ID、文件名；当前业务没有对应维度时可留空，但不要从文件名反推。
- 爬取接口数据并重组成表格时，最终文件固定为 `.csv`，使用 `encoding="utf-8"` 和 `newline=""` 写入。
- 非系统自生成的文本、CSV、JSON、字段映射、任务状态和脚本文件统一使用 UTF-8 编码；服务端原始下载文件按响应二进制原样保存，不强行转码。
- 下载报表或二进制文件时，保留服务端原始后缀，例如 `.xlsx`、`.csv`、`.zip`、`.pdf`；只按上述规则规范文件名前缀，不强制改成 `.csv`。
- 同一脚本包含多个采集请求任务、多个报表或多个业务功能时，必须按请求任务或业务导出表名称拆分成多个输出文件；除非用户明确要求汇总，不要把多个业务输出合并成同一个 CSV。
- 多子任务状态记录里的 `output_file` 必须记录实际生成的拆分文件路径，重跑跳过已完成任务时同时检查该输出文件存在。

## 生成脚本形态

生成的代码保持直接，并强制按 Ponytail 规范收敛：先判断代码是否需要存在，优先标准库和已允许的 `requests`，用最少文件、最少函数、最少状态完成当前需求。默认脚本形态固定如下。
...
- 对应页面操作、接口请求、导出链路或子任务编排的 `class`、函数或方法，注释必须包含“操作步骤”和“访问网址”；“操作步骤”必须写页面上点击哪里、选择哪里、如何筛选或导出才能触发该请求，用 `-` 分割步骤，不写接口内部组参、请求、分页累加这类代码步骤；“访问网址”必须写浏览器地址栏显示的页面 URL，不写接口 path 或 API URL。没有外部访问网址的本地处理方法，只写职责，不强行补 URL。
- 采集请求方法必须在方法体开头用三引号写参数注释，格式为 `参数：参数名 对应功能，类型，是否必填，格式/来源；...`。实际 `query`、`json`、`form data` 或分页参数构造前，也要用三引号说明请求参数对应功能、类型、是否必填和来源。
- `class` 数量默认控制为 1 个，除非业务复杂到必须拆分
- `class` 只为组织当前脚本职责服务，不要演变成通用框架
- 默认只保留一个业务 `class` 加 `update_headers()`、`fetch_*`、`build_*`、`write_*`、`run()` 这类少数方法；能顺序写在当前方法里的逻辑不要再拆。
- 超过 4 个主要方法、超过 1 个业务类或新增独立文件时，必须有真实复用、独立认证/签名链、多子任务状态或明确业务边界作为理由。
...
2. 关键参数包括但不限于：数据抓取时间、业务日期、业务导出表名称、店铺 ID、店铺名、cookie、token、日期范围、分页大小、导出路径、输出文件名；只有表格导出场景才包含字段映射路径。
...
- 宁可在一个 `fetch_*` 方法里顺序写完同一条请求链，也不要为了“看起来分层”拆出一串只调用一次的小方法。
...
- 生成表格文件名按 `数据抓取时间-业务日期-业务导出表名称.<suffix>` 命名，业务维度写入文件内容，不写入文件名；爬取数据写 UTF-8 CSV，下载报表保留原始后缀
- 表格导出场景下，读取独立的 `field_mapping.json`，不要在 `.py` 里再维护重复映射
- 生成代码时必须包含至少 1 个 `class`
- 每个函数和每个方法上方都要有简短注释
- 对应页面操作、接口请求、导出链路或子任务编排的类/方法注释要写明页面点击路径和页面访问网址，操作步骤用 `-` 分割，访问网址写浏览器地址栏页面 URL，不写接口 path
- 采集请求方法和请求参数构造处要用三引号写参数注释，说明参数对应功能、类型、是否必填、格式或来源
...
- 强制遵守 Ponytail：最短可维护代码优先，不生成未请求的抽象、配置层、通用框架或“以后可能用”的文件
- 交付前用浏览器结果校验脚本输出
...
- 抓包阶段是否先用 chrome-devtools-mcp 附着和验证现有标签页，且只接管了已有浏览器
- 如果 chrome-devtools-mcp 只看到新的 `about:blank`、空白页或孤立受管会话，是否已经停止该路径并改走直接 CDP
...
- 返回实际生成的业务输出文件路径；同一脚本有多个采集请求任务时，逐个列出拆分后的输出文件
