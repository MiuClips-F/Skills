...
MCP/CDP 只用于前期抓包和请求链分析，最终交付的 Python 脚本不得包含任何 CDP 接管、9222 检查或浏览器调试参数。
...
1. 连接已有 Chrome 的 `127.0.0.1:9222` 调试端口。
2. 先检查 `http://127.0.0.1:9222/json` 或 `http://127.0.0.1:9222/json/version` 是否可用，只接管已打开的浏览器和已有标签页，禁止新开浏览器或新起 Chrome 实例。
...
5. 仅在确认已附着到用户现有标签页后，再用可用的页面工具或直接 CDP 执行用户描述的操作。优先用页面快照定位元素，再执行点击、填写、上传、按键和等待。
6. 在每一步操作期间抓包，保留请求方法、URL、query、body、请求头、响应状态、响应体、跳转、下载地址和关键时序。
