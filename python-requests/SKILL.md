...
- 子任务状态文件使用 JSON，但不得默认写到脚本同级目录或固定 `task_state.json`；状态文件完整路径必须由 `run()` 或 `main()` 参数传入，不默认保存到 `self`。
...
- 每个业务输出文件名必须使用 `<yyyyMMddHHmmss>-<业务导出表名称>.<suffix>`；`yyyyMMddHHmmss` 是数据抓取日期时间，按 `%Y%m%d%H%M%S` 生成或由调用方明确传入。
- 文件名不得承载筛选维度或业务关键字段；数据日期、店铺 ID、店铺名、商品 ID、SKU、商品名称、直播间名称、直播场次 ID、账户名、账户 ID 等关键字段，只要当前业务存在，就必须写入文件内容的列或原始下载内容中，不能只写在文件名或目录名中。
- 业务导出表名称作为文件名段，保留用户能识别的名称；只清理文件系统不允许的字符。
- 输出目录和完整输出文件路径都必须由 `run()`、`main()` 或 `write_*()` 参数传入，包括文件名本身；脚本不得默认使用脚本同级 `output` 目录，也不得在同级目录自动生成一批产物。
- 爬取接口数据并重组成表格时，最终文件固定为 `.csv`，使用 `encoding="utf-8"` 和 `newline=""` 写入。
- 下载报表或二进制文件时，保留服务端原始后缀，例如 `.xlsx`、`.csv`、`.zip`、`.pdf`；只按上述规则规范文件名，不强制改成 `.csv`。
- 同一脚本包含多个采集请求任务、多个报表或多个业务功能时，必须按请求任务或业务功能拆分成多个输出文件；除非用户明确要求汇总，不要把多个业务输出合并成同一个 CSV。多个输出文件的完整路径也必须由调用方参数传入，不能在脚本内部按店铺或报表自行拼接。
- 多子任务状态记录里的 `output_file` 或 `output_paths` 必须记录实际生成的拆分文件路径，重跑跳过已完成任务时同时检查该输出文件存在。
...
- 任务状态 JSON，只有一个大任务包含多个可独立完成的子任务，且需要断点重跑/跳过已完成任务时才写入用户传入的 `state_file_path`
...
2. 关键参数包括但不限于：数据日期、店铺 ID、店铺名、商品 ID、SKU、商品名称、直播间名称、直播场次 ID、账户名、账户 ID、cookie、token、日期范围、分页大小、业务导出表名称、完整输出文件路径、状态文件路径；只有表格导出场景才包含字段映射路径。
3. 当用户明确要求本地可直接测试时，应把当前抓包或当前页面使用的值整理为测试参数写在 `__main__` 中。
4. 文件名、输出路径这类动态参数也应写在 `__main__` 或命令行参数解析中，不要藏在函数内部临时生成；`run()`、`write_outputs()`、状态写入方法必须接收完整路径参数。字段映射路径仅在表格导出场景加入。
...
```json
{
  "数据日期": "__context__.data_date",
  "店铺ID": "__context__.shop_id",
  "店铺名称": "__context__.shop_name",
  "商品ID": "__context__.product_id",
  "SKU": "__context__.sku",
  "商品名称": "__context__.product_name",
  "直播间名称": "__context__.live_room_name",
  "直播场次ID": "__context__.live_session_id",
  "账户名": "__context__.account_name",
  "账户ID": "__context__.account_id",
...
- 采集大任务输出文件名按 `<yyyyMMddHHmmss>-<业务导出表名称>.<suffix>` 命名，完整输出路径和状态文件路径由参数传入；爬取数据写 UTF-8 CSV，下载报表保留原始后缀
- 数据日期、店铺 ID、店铺名、商品 ID、SKU、商品名称、直播间名称、直播场次 ID、账户名、账户 ID 等关键字段要写入输出文件内容，不能只体现在文件名或目录名中
...
- 返回实际生成的业务输出文件路径；同一脚本有多个采集请求任务时，逐个列出由参数传入的拆分输出文件
- `__main__` 中当前测试参数的说明，例如数据日期、店铺、商品、直播间、账户、cookie、日期范围、完整输出路径、状态文件路径；如果是表格导出，再补充字段映射路径
- 认证参数的来源说明：
  - 哪些是脚本自行生成
  - 哪些来自前置接口
  - 哪些需要用户提供
